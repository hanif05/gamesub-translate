...
- **`Sdcb.PaddleInference.runtime.win64.cu120-sm61-75` harus direct ref (mengganti `Paddle.Runtime.win_x64`).** Sama gotcha: native DLL (`paddle_inference.dll`, CUDA deps, dll) cuma ter-copy ke output waktu `<PackageReference>` langsung di csproj, bukan transitif. Kalau project cuma reference `Sdcb.PaddleOCR` (transitively), runtime bakal `DllNotFoundException`. Fix: tambah direct `<PackageReference Include="Sdcb.PaddleInference.runtime.win64.cu120-sm61-75" Version="2.6.1" />` di Core + Prototype csproj. Versi runtime + `Sdcb.PaddleOCR` (2.7.0.3) + `Sdcb.PaddleOCR.Models.Online` (2.7.0) harus aligned — kalau OCR 2.7.0.x di-pair dengan Models.Online 2.5.0 (NuGet default), runtime `MissingMethodException: FileDetectionModel..ctor(string)` (signature ctor beda). Validated di T90.
- **PaddleOCR pakai `Sdcb.PaddleOCR`, bukan raoyutian PaddleOCRSharp.** Free raoyutian NuGet adalah CPU-only (banner native bilang "当前使用的是CPU版本" — use_gpu flag di-ignore, silent fallback CPU). Untuk GPU yang beneran kepake, pakai Sdcb (Apache-2.0, free CUDA via cu*_sm* runtime). API beda total: `PaddleOcrAll` + `PaddleDevice.Gpu()`/`Mkldnn()` (returns `Action<PaddleConfig>` lambda) + `engine.Run(Mat)` (bytes di-decode dulu via `Cv2.ImDecode`). Lihat T90.
- **`UseWindowsForms=true` bikin type ambigu di WPF.** Global using `System.Windows.Forms` otomatis aktif → `TextBox`, `Button`, `ComboBox`, `Brush`, `MessageBox`, `Brushes`, `Cursors`, `KeyEventArgs` collide dengan WPF. Fix: tambah alias eksplisit di file yang kena (lihat `SettingsWindow.xaml.cs` header). Ini gotcha yang sama sudah ada di `App.xaml.cs` (fully-qualified `System.Windows.*`) dan di Core project (`using Timer = System.Threading.Timer;` di TesseractOcrEngine + PaddleOcrEngine).
- **GPU PaddleOCR butuh cuDNN di PATH.** Native `paddle_inference.dll` sukses load + bind CUDA, tapi `cudnn64_8.dll` (atau `cudnn64_9.dll` untuk runtime cu126+) harus ada di `%PATH%`. Tanpa itu runtime throw `PreconditionNotMetError: cudnn64_8.dll ... not configured correctly (error code 126)`. Fix: install CUDA Toolkit 12.x + cuDNN, atau extract `cudnn64_*.dll` ke `C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v12.x\bin\`. CPU path (PaddleDevice.Mkldnn) gak butuh cuDNN — jalan out-of-the-box (validated T90 warm median 79ms).
