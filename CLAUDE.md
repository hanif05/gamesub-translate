...
- **`Paddle.Runtime.win_x64` harus direct ref.** NuGet ini cuma punya `<None Include="$(MSBuildThisFileDirectory)\win_x64\*.*">` di targets file → MSBuild cuma load targets waktu ada `<PackageReference>` langsung di csproj, bukan transitif. Kalau project cuma reference `PaddleOCRSharp`, native DLL (`paddle_inference.dll`, `opencv_world470.dll`, `mkldnn.dll`, dll) tidak akan ke-copy ke output → runtime error `DllNotFoundException: PaddleOCR`. Fix: tambah `<PackageReference Include="Paddle.Runtime.win_x64" Version="..." />` di project yang sama dengan `PaddleOCRSharp` (saat ini Core project).
